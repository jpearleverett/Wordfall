...
const BOUNDS: [number, number, number][] = [
  [0, 0.30, 18],  // First Sprout  — FIXED, measured 22% / 21
  [1, 0.55, 11],  // Forest Walk   — status quo, measured 49% / 12
  [2, 0.70, 11],  // Garden Bloom  — status quo, measured 64% / 12
  [3, 0.69, 11],  // Wild Meadow   — status quo, measured 63% / 12
  [4, 0.69, 11],  // Mountain Peak — status quo, measured 63% / 15
  [5, 0.52, 11],  // Lab Basics    — status quo, measured 46% / 12
  [6, 0.64, 11],  // Chemistry Set — status quo, measured 58% / 12
];
...
describe('find-list variety across a chapter', () => {
  ...
  it('chapter 1 is measurably better than the twelve-word list it replaced', () => {
    ...
    try {
      ...
      expect(before.carryOver).toBeGreaterThan(0.30);

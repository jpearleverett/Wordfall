...
| `src/engine/solver.ts` | 8-directional DFS word finder, recursive backtracking solver, dead-end detection, hint generation. Mode-aware variants: `isSolvable` (standard gravity), `isSolvableGravityFlip` (rotating gravity), `areAllWordsIndependentlyFindable` (noGravity). `findWordInGrid` supports optional `limit` parameter for early termination. `getHint` uses solution ordering directly without redundant re-solve |

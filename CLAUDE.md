...
### Monetization curve overhaul (August 2026 — `claude/word-puzzle-monetization-kqd5nw`)

A full L1–1000 revenue audit (economy math + market benchmarks, adversarially verified) found the core loop competitive but scarcity, spend pressure, and difficulty framing all decaying to zero past ~L30. This branch implements the ranked fixes end to end:

- **Economy re-tightened.** Login calendar gems 299 → 96/cycle, coins 8,475 → 6,000 (week-1 honeymoon untouched; totals pinned by test); weekly-goal rewards halved; late streak tiers trimmed (30/45/60/100d, cosmetics kept). New `claimMeteredGems(amount, source)` aggregate daily gem cap (RC `dailyTotalGemCap`, default 10) on the `claimFlawlessGems` ledger pattern, wired at daily-quest / wheel / reward-timer grant sites in App.tsx. Weekly goals deliberately NOT metered (their ceremony channel is shared with one-time seasonal quest steps).
- **Rescue pricing floors.** hint_rescue 5-for-50c → 3-for-80c, post_puzzle 10-for-80c → 5-for-150c, post-loss pack 3-for-120c; grant counts live in `OFFER_HINT_GRANTS` and locale strings interpolate `{{count}}` (display==grant). Rescue multiplier ladder continues past the frozen 'expert' label: x6 at L151+, x8 at L601+ (`getRescuePriceMultiplier`).
- **Store bridges completed.** `MiniPackNeed` gains `'undo'` (empty undo tap opens the sheet instead of silently no-oping); Time Pressure gains a gem +30s continue (`TIMEOUT_CONTINUE_GEM_COST` = 12) once the per-attempt ad continue is spent; new First-Letter Peek partial hint (40c, `USE_PARTIAL_HINT` action, first row of the hints mini-pack — charge+delivery in one GameScreen handler).
- **Churn valves unblocked.** `handleRetry` resets `stuckFailRecordedRef` (dead-end→retry→dead-end now records both failures); a stuck-loop valve surfaces `FailBreatherOffer` in the modes whose dead-end never sets `status='failed'` (classic et al) on the 2nd recorded struggle, status-aware so a live board never gets a failed overlay; one-time "clear the word on TOP first" banner at L31 (the 12%→57% regime onset).
- **Curve fixed past 600.** Procedural tail seeds from the curated endgame (L601 ≥ L600, expert from chapter 41; the old 'hard' on-ramp served L601–675 easier than L100); scaled default branch (no more level-11 boards at L905); breather-in-breather-chapter double-ease replaced by a one-word dip from the chapter's own textured config; texture cycle 4 → 7 silhouettes (repeat period 300 → 525 levels); wing finales flagged `isBossChapter`; wing ceremonies now pay past L600 (`procedural_N` wings, "Endless Wing N").
- **Boss words.** `WORDS_7` bucket (283 curated words); spikes past `BOSS_WORD_MIN_LEVEL` (300) may reach length 7 — the built-but-unreachable "{N} LETTERS!" celebration now fires. Plain levels keep 3–6 (pinned).
- **Pinch levels.** `isPinchLevel` (penultimate level of each chapter past L150, RC `pinchLevelsEnabled`); `generatePinchBoard` inverse-shops for the lowest forgiveness in [0.05, 0.35] (solvable always — one-ply lookahead wins any solvable board); `generateLevelBoard` routes the classic call sites; pinch slots are exempt from the breather/adaptive easer and share the spike CHALLENGE framing + booster sheet.
- **Decoy knob.** `BoardConfig.decoyRichness` (0..1) shifts filler toward vowels/common consonants (visual-search difficulty for the capped tail, ramped post-600, cap 0.5/0.25 by weight for the perf budget) with a filler-copy guard that surgically overwrites one filler cell per accidental copy post-solvability. Fixed a latent `findWordInGrid` bug: completed paths were pushed once per neighbor direction of the last cell (occurrence counting was meaningless; noGravity limit-50 path lists were mostly duplicates).
- **Spend surfaces added.** `PreLevelBoosterSheet` on spike/pinch entry (RC `preLevelBoosterSheetEnabled`, once per level, stocks — never auto-equips); victory-screen pursuit chips (nearest daily quest via the same template matchers, piggy-bank fill); milestone drumbeat extended past its L150 dead end via `getScheduledLevelBonus` (consumable-weighted, currency confined to centuries, `rewardCadence` guard now runs to L1000); `OutOfEnergyModal` replaces the bare Alert at all three soft-energy walls; SeasonPassHomeCard "finish your track" nudge (≤10 tiers, ≤7 days) into the buried gem tier skip.
- **Live-ops.** EventScreen "FINISH IT" gem buyout of an event ladder's last stretch (last 25% or final 24h, ~1 gem/1%, clamp 5–80, RC `eventBuyoutEnabled`); weekend tournament Fri 17:00→Sun 22:00 UTC in hashed ~100-player brackets riding the per-event leaderboard rails (RC `weekendTournamentEnabled`, `tournamentBracketCount`).
- **Ad pacing.** Interstitials 5→6/day, 90s→75s, min level 13→10; offers 3→4/session, cooldown 8→6min; zero-tap auto-advance no longer ambushes an interstitial (RC `interstitialOnAutoAdvance`, default off — PuzzleComplete passes `{auto:true}`); Remove-Ads consumable auto-grants get an RC 'scoped' mode (3/day; default 'legacy' so paid behavior only changes by experiment).
- **Deleted dead content:** `data/missions.ts` (zero importers; carried impossible combo/Cascade/Limited-Moves templates), `data/sideObjectives.ts` (zero importers, meaningless 'par'); Sports category triplicate 'BOUT' + plural fillers replaced.

...
- **Monetization:** IAP via react-native-iap (17 in shopProducts.ts + 3 mega bundles in dynamicPricing.ts = 20 total, incl. VIP weekly subscription, $49.99 Royal Collection, $99.99 Ultimate Whale Pack), rewarded + interstitial ads via AdMob (mock fallback), gem-based contextual offers wired to all 6 triggers with analytics, dynamic pricing by segment with flash sales via `getFlashSale()`, regional pricing, season pass, cosmetic store
...
│   ├── shopProducts.ts      # 17 IAP product definitions with rewards, categories, store IDs (incl. VIP weekly subscription, $49.99 Royal Collection, $99.99 Ultimate Whale Pack)
...
| `src/data/coinShop.ts` | 13 coin-purchasable consumables across 3 categories (boosters, consumables, temporary). Items: hints, undos, spotlight, shuffle, wildcard, 2x XP, lucky charm (+rare tile chance), theme rental, premium hint, board freeze, score doubler. Daily purchase limits. `canPurchaseCoinItem()`, `getCoinShopByCategory()` |
...
| `src/data/shopProducts.ts` | 17 IAP product definitions with store IDs, rewards, categories (including `vip_weekly` subscription, `royal_collection` at $49.99, `ultimate_whale` at $99.99). Helpers: `getProductById()`, `getProductRewards()`, `getProductsByCategory()` |
...
- **App Store / Play Store IAP products** — register 20 product IDs (17 in shopProducts.ts incl. VIP weekly + whale tiers + 3 mega bundles, prefixed `wordfall_`) in store consoles. Without store config, IAP runs in mock mode

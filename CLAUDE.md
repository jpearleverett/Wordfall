...
- **Monetization:** IAP via react-native-iap (17 core + 3 mega bundle + 2 whale-tier products = 22 total, incl. VIP weekly subscription, $49.99 Royal Collection, $99.99 Ultimate Whale Pack), rewarded + interstitial ads via AdMob (mock fallback), gem-based contextual offers wired to all 6 triggers with 5-minute FOMO countdown and analytics, dynamic pricing by segment with flash sales, regional pricing, season pass, cosmetic store
...
│   ├── ReferralCard.tsx                         # Referral invite card with code copy, share, milestone progress
...
│   ├── shopProducts.ts      # 20 IAP product definitions with rewards, categories, store IDs (incl. VIP weekly subscription, whale tiers at $49.99/$99.99)
...
│   ├── coinShop.ts          # 13 coin-purchasable consumables (hints, boosters, XP boosts, theme rentals, premium hint, board freeze, score doubler)
...
| File | Purpose |
|------|---------|
| `App.tsx` | Entry point (~1,648 lines, decomposed from 2,024). Wrapped in `ErrorBoundary`. Progressive tab unlocking, nested stack navigators, provider wrappers. Reward wiring delegated to `useRewardWiring` hook, ceremony queue to `useCeremonyQueue` hook. Still contains: welcome-back modal, GameScreenWrapper, navigation definitions, feature unlock detection |
...
| `src/screens/HomeScreen.tsx` | Dynamic home screen with `VideoBackground`, image-based UI, progressive section visibility based on `playerStage` AND `playerSegmentation`. Sections: hero card, streak, daily rewards, Mystery Wheel button (with free spin badge + "DAILY FREE!" gold badge + pulse animation), event banners, weekly goals, missions, recommendations, quick play, pending challenge cards, pending gift banner. Section visibility driven by segment-aware `segmentHomeContent` when available |
...
| `src/utils/shareGenerator.ts` | `generateShareText()` for Wordle-style emoji grid with CTA, `generateChallengeShareText()` for competitive challenge sharing, `generateStreakCard()` for shareable streak display with "Join me on Wordfall!", `generateCollectionCard()` for collection progress sharing |
...
| `src/services/iap.ts` | IAP service via `react-native-iap`. 15 products across bundles/consumables/currency/premium/subscription. Mock mode for dev/Expo Go. NativeModules check on init prevents EventEmitter crash when native module isn't linked. Receipt storage, restore purchases, parental control integration. Wire to ShopScreen for real purchase flow |
...
| `src/data/coinShop.ts` | 10 coin-purchasable consumables across 3 categories (boosters, consumables, temporary). Items: hints, undos, spotlight, shuffle, wildcard, 2x XP, lucky charm (+rare tile chance), theme rental. Daily purchase limits. `canPurchaseCoinItem()`, `getCoinShopByCategory()` |
...
| `src/data/shopProducts.ts` | 20 IAP product definitions with store IDs, rewards, categories (including `vip_weekly` subscription, `royal_collection` at $49.99, `ultimate_whale` at $99.99). Helpers: `getProductById()`, `getProductRewards()`, `getProductsByCategory()` |
...
- 73+ cosmetic items (17 themes, 25 frames, 26 titles, 24 decorations) — many now coin-purchasable as coin sinks
...
- **VIP weekly subscription**: $4.99/week subscription product in `shopProducts.ts`. Benefits: ad-free, 50 daily gems, 3 daily hints, exclusive VIP frame, 2x XP. Economy integration with `isVipSubscriber`, `vipExpiresAt`, `claimVipDailyRewards()`. Prominent VIP card at top of ShopScreen with ACTIVE/SUBSCRIBE states and daily claim button
...
- **Referral system**: 6-char referral codes generated from user ID hash. Referrer rewards: 500 coins + 10 gems per referral. Referred rewards: 200 coins + 5 gems + 3 hints. Milestones: 3 referrals (1000 coins), 5 (exclusive frame), 10 (exclusive title). `ReferralCard.tsx` glassmorphism component for HomeScreen with code copy, Share API, milestone progress
...
- **App Store / Play Store IAP products** — register 18 product IDs (15 core incl. VIP weekly subscription + 3 mega bundles, prefixed `wordfall_`) in store consoles. Without store config, IAP runs in mock mode
...
- End-to-end testing (unit tests at 12 files/274 tests, but no E2E)
- Deep linking (needed for referral system to work cross-device)
...
- **Portrait orientation only** (set in app.json). App configured with deep linking scheme `wordfall://`, iOS privacy manifests (UserDefaults + DiskSpace), Android permissions (INTERNET, VIBRATE, RECEIVE_BOOT_COMPLETED, SCHEDULE_EXACT_ALARM), OTA updates via expo-updates, auto-increment build numbers for store submissions
...
- **IAP** in `src/services/iap.ts` uses `react-native-iap`. 15 products (14 core + VIP weekly subscription). Mock mode auto-activates in dev/Expo Go. Init checks `NativeModules` for the IAP native module before importing to prevent EventEmitter crash. Parental controls enforced before every purchase via SettingsContext

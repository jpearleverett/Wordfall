...
- **AdMob real IDs** — `react-native-google-mobile-ads@^16` is installed + autolinked via the config plugin in `app.json` (today using Google's public test app IDs). `AD_CONFIG` in `constants.ts` defaults to Google's public test rewarded + interstitial ad unit IDs so dev client builds show real test ads. To go live: swap `androidAppId` / `iosAppId` in the `app.json` plugin block for real AdMob app IDs, set `EXPO_PUBLIC_ADMOB_REWARDED_ID` + `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ID` in `.env`, rebuild EAS APK. Without the native module (Metro-only / Expo Go paths), `MockAdModal` simulates the ad UI in `__DEV__`
...
- **AdMob native SDK installed with test IDs** — `react-native-google-mobile-ads@^16` is installed and autolinked via the config plugin in `app.json`. The plugin is configured with Google's public test app IDs (`ca-app-pub-3940256099942544~...`) so `expo prebuild` produces a valid native project; `AD_CONFIG.REWARDED_AD_UNIT_ID` / `AD_CONFIG.INTERSTITIAL_AD_UNIT_ID` default to Google's public test unit IDs. `ads.ts` calls `trackAdWatched` on every successful ad completion and adds Sentry breadcrumbs on every failure path. `MockAdModal` still handles the `__DEV__` path when the native module is unavailable (e.g. Metro-only reload against a stale APK). Production swap: real `androidAppId` / `iosAppId` in `app.json`, real `EXPO_PUBLIC_ADMOB_REWARDED_ID` / `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ID` in `.env`, fresh EAS build
...
- **Ads** in `src/services/ads.ts` supports rewarded + interstitial via `react-native-google-mobile-ads@^16` (installed + autolinked). Google's public test app + unit IDs are the current defaults so dev builds show real test ads; `MockAdModal` fallback handles `__DEV__` when the native module isn't linked. `isAdFree` flag in EconomyContext disables all ads. Interstitials have separate daily cap (5) and minimum interval (90s). `AD_CONFIG` in constants.ts has `MAX_INTERSTITIALS_PER_DAY`, `INTERSTITIAL_INTERVAL_MS`, `REWARDED_AD_UNIT_ID`, and `INTERSTITIAL_AD_UNIT_ID`. Ad completion fires `analytics.trackAdWatched`; ad failures are breadcrumbed via `crashReporter` for Sentry context

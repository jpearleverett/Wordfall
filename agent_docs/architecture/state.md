...
- **Game state:** `useGame` hook creates a **zustand store** via `createGameStore()` (in `src/stores/gameStore.ts`) which wraps the existing `gameReducer` using zustand's `redux` middleware. `useGame` returns `{ store, ...actionDispatchers, isStuck, stars, foundWords, totalWords, remainingWords }` — **it does NOT return `state`**. Consumers read state via `useStore(store, selector)` for narrow subscriptions. 22 reducer actions: SELECT_CELL, SELECT_CELLS (batched), CLEAR_SELECTION, SUBMIT_WORD, USE_HINT, UNDO_MOVE, NEW_GAME, HYDRATE_FROM_SNAPSHOT, TICK_TIMER, USE_BOOSTER, GRANT_HINT, GRANT_UNDO, GRANT_BOOSTER, USE_PREMIUM_HINT, SMART_SHUFFLE, SPOTLIGHT_ACTIVATE, ACTIVATE_SCORE_DOUBLER, ACTIVATE_BOARD_FREEZE, ACTIVATE_BOOSTER_COMBO, EXPIRE_BOOSTER_COMBO, MARK_FAILED. (RESET_COMBO + INCREMENT_CHAIN were ripped in Apr 2026 with the Option A refactor — see `agent_docs/game_mechanics.md`.) GameScreen subscribes to ~20 coarse selectors (status, score, perfectRun, moves, etc.) that change per-word, NOT per-tap. PlayField subscribes to `selectedCells` (per-tap). This separation is the primary latency optimization.
...
- **Player data:** `PlayerContext` - progress, collections (atlas/tiles/stamps), missions, streaks, cosmetics, library wings, mode stats, achievements, comebacks, **plus**: `featuresUnlocked`, `weeklyGoals`, `pendingCeremonies`, `tooltipsShown`, `failCountByLevel`, `consecutiveFailures`, `lastBreatherOfferedAt` (Tier 6 B1 — 1-hour cooldown marker for the fail-breather offer), `mysteryWheel`, `winStreak`, `puzzleEnergy`, `performanceMetrics`, `segments` (now includes `daysSinceActive` per Tier 6 B6 for comeback-ladder tiering), `eventProgress`, `friendChallenges`, `modeLevels`, `seasonalQuest`, `prestige`. Methods include `useEnergy`, `refillEnergy`, `recomputeSegments`, `updateEventProgress`, `sendChallenge`, `respondToChallenge`, `recordPerformanceMetrics`, `advanceModeLevel`, `getModeLevel`, `updateSeasonalQuest`. **Persisted to AsyncStorage with 300ms debounce** + AppState flush + Firestore sync when configured.
- **Prestige permanent bonuses (Tier 6 B3 — shipped April 2026):** `src/data/prestigeSystem.ts` exports three multiplier helpers — `getPrestigeXpMultiplier(prestigeLevel)`, `getPrestigeCoinMultiplier(permanentBonuses)`, `getPrestigeGemMultiplier(permanentBonuses)` — that decode the accumulated `permanentBonuses` string IDs on `PrestigeState`. `useRewardWiring.ts:280–300` composes `cosmetic × prestige` multipliers multiplicatively and caps the composed factor at `MAX_BONUS_FACTOR = 5.0`. Prior to Tier 6 these multipliers were declared but never read — the permanent-bonus system was cosmetic theater. `ProfileScreen.tsx` surfaces the live "1.50× XP · 1.25× Coin · 2.00× Gem" summary on the prestige badge.
...
- **GameScreen does NOT subscribe to `selectedCells`** — it reads ~20 coarse zustand selectors (status, score, perfectRun, moves, remainingWords, etc.) that change per-word, not per-tap. When a cell is tapped, only PlayField + ConnectedWordBank re-render (~15-30ms), not the full 2500-line GameScreen.
...
- **GameFlashes** (`src/screens/game/GameFlashes.tsx`) — overlay blocks (valid/invalid flash, score popup, big-word label) extracted as `React.memo`. Receives Animated.Values by stable ref + primitive props. Memo bails out on every tap. The chain popup / neon pulse / VHS glitch overlays were deleted in the Option A refactor (April 2026).

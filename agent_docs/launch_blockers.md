...
- **Current:** every board in the game — including level 1 — is
  procedurally generated by `boardGenerator.ts` from a Mulberry32 seed
  (`level * 1337`). What's hand-curated is chapter *metadata* for IDs
  1–40 in `src/data/chapters.ts` (name, theme, icon, 12 theme words,
  star gate, `GenerationProfile`). Past level 600, `getChapterForLevel`
  falls through to `generateProceduralChapter()` in
  `puzzleGenerator.ts:300`, which cycles `WORD_CATEGORIES` and rolls
  names from a 20×20 adj×noun table. The `chapterOverrideJson` Remote
  Config path exists for publishing hand-curated chapter metadata for
  IDs 41+ without a client rebuild.
- **Why this matters:** there is **no hard puzzle cap** — the "600"
  number is just the `CURATED_CHAPTER_COUNT × PUZZLES_PER_CHAPTER`
  constant. The tail is playable indefinitely. What degrades past 600
  is curated-feel: generic chapter names ("Emerald Spire — Nature

...
- **Procedurally generated boards, hand-curated chapter metadata**: every board in the game — including level 1 — is built at runtime by `generateBoard(config, seed, mode)` from a Mulberry32 seed derived from the level number. There are **zero hand-placed grids** in the repo. What's "authored" is the metadata for chapters 1–40 (`src/data/chapters.ts`): chapter name, theme, description, icon, 12 theme words that bias `selectThemedWords()`, `requiredStars` gate, and a `GenerationProfile` (grid size range, word length range, introduced mechanics, empty-cell density, dictionary tier). The 15 puzzles inside each chapter come from that profile + per-level seeds. Past level 600 the pipeline falls through to `generateProceduralChapter()` in `src/engine/puzzleGenerator.ts:300` — it cycles `WORD_CATEGORIES`, rolls a chapter name from a 20×20 adjective×noun table, and scales difficulty via `getProceduralBoardConfig()` (caps at 10×8 grid / 10 words). Ops can publish hand-curated chapter metadata for IDs 41+ via the `chapterOverrideJson` Remote Config path without a client rebuild. **There is no hard puzzle cap** — the "600" number is just the `CURATED_CHAPTER_COUNT × PUZZLES_PER_CHAPTER` constant at which the curated-metadata catalog ends.
